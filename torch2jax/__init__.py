  ...
  def view(self, *shape):
    if len(shape) == 1:
      if isinstance(shape[0], Sequence):
        shape = shape[0]
      elif isinstance(dtype := shape[0], torch.dtype):
        return Torchish(self.value.astype(t2j_dtype(dtype)))
      else:
        raise ValueError(f"Tensor.view takes shape or dtype, got {shape[0]}")
